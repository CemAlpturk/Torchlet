...
from torchlet import scalar_ops as ops
...
class MathTest(Generic[A]):
    ...
    def addConstant(a: A) -> A:
        """Add a constant to the argument"""
        return 5 + a
    ...
    def log(a: A) -> A:
        ...
        return ops.log(a + 100000)

    @staticmethod
    def relu(a: A) -> A:
        """Apply relu"""
        return ops.relu(a + 5.5)

    @staticmethod
    def exp(a: A) -> A:
        """Apply exp to a smaller value"""
        return ops.exp(a - 200)

    @staticmethod
    def explog(a: A) -> A:
        return ops.log(a + 100000) + ops.exp(a - 200)
    ...
    @staticmethod
    def gt2(a: A, b: A) -> A:
        return ops.lt(b, a + 1.2)

    @staticmethod
    def lt2(a: A, b: A) -> A:
        return ops.lt(a + 1.2, b)

    @staticmethod
    def eq2(a: A, b: A) -> A:
        return ops.eq(a, (b + 5.5))
    ...
    @staticmethod
    def complex(a: A) -> A:
        return ops.log(ops.sigmoid(ops.relu(ops.relu(a * 10 + 7) * 6 + 5) * 10)) / 50

    @classmethod
    def _tests(cls) -> tuple[
