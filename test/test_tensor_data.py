...
from torchlet.tensor_data import TensorData, IndexingError, shape_broadcast
...
def test_enumeration(tensor_data: TensorData) -> None:
    """Test enumeration of tensor datas."""
...
def test_index(tensor_data: TensorData) -> None:
    ...
    # Check that all indices are whithin the size.
    for ind in tensor_data.indices():
...
def test_shape_broadcast() -> None:
    c = shape_broadcast((1,), (5, 5))
    assert c == (5, 5)

    c = shape_broadcast((5, 5), (1,))
    assert c == (5, 5)

    c = shape_broadcast((1, 5, 5), (5, 5))
    assert c == (1, 5, 5)

    c = shape_broadcast((5, 1, 5, 1), (1, 5, 1, 5))
    assert c == (5, 5, 5, 5)

    with pytest.raises(IndexingError):
        c = shape_broadcast((5, 2), (5,))
        print(c)

    with pytest.raises(IndexingError):
        c = shape_broadcast((5, 7, 5, 1), (1, 5, 1, 5))
        print(c)
